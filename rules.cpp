...
rules::rules(sf::Vector2i offset, sf::Vector2f visibility) : pile(offset), fannedPile(offset, fannedPile::Down, visibility) {
}

bool rules::canSplit(unsigned int cardIndex) {
	return commonSplitTest(cardIndex);
}

bool rules::canAppend(const pile& p) {
